...
namespace RezRouting.AspNetMvc
{
    ...
    public static class MvcRouteMapperExtensions
    {
        ...
        public static void MapMvcRoutes(this RouteMapper mapper, RouteCollection routes, Action<ResourcesModel> modelAction = null)
        {
            if (mapper == null) throw new ArgumentNullException("mapper");
            if (routes == null) throw new ArgumentNullException("routes");

            var model = mapper.Build();
            new MvcRouteCreator().CreateRoutes(model, routes);
            if (modelAction != null)
                modelAction(model);
        } 
    }
